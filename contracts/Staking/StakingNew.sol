...
contract StakedContract is Ownable {
    ...
    modifier checkPeriod(uint64 _period) {
        require(_period == 7 || _period == 30 || _period == 90 || _period == 180, "Staking:: Unavailable period for staking");
        _;
    }

    modifier checkMaxDepositByPeriod(uint256 _amount, uint64 _period) {
        if (_period == 7) {
            require(_amount <= 200 * (10**18), "Staking:: Max deposit 200 LOTT");
        }
        if (_period == 30 || _period == 90 || _period == 180) {
            require(_amount <= 25000 * (10**18), "Staking:: Max deposit 25000 LOTT");
        }
        _;
    }
    ...
    function _calculateUnlockTime(uint64 _period) internal view returns(uint256) {
        uint256 unlockTime;
        if (_period == 7) unlockTime = block.timestamp + 7 days;
        else if (_period == 30) unlockTime = block.timestamp + 30 days;
    ...
    function _calculateRewardPrecent(uint64 _period) internal pure returns(uint256) {
        uint256 _precent = 0;
        if (_period == 7) _precent = 276712345238095240;
        else if (_period == 30) _precent = 41666666666666664;
        else if (_period == 90) _precent = 55555555555555550;
        else if (_period == 180) _precent = 69444444444444450;
    ...
    function _calculateRewardPrecentForPeriod(uint64 _period) internal pure returns(uint256) {
        uint256 _precent = 0;
        if (_period == 7) _precent = 101000000000000000000;
        else if (_period == 30) _precent = 15000000000000000000;
        else if (_period == 90) _precent = 20000000000000000000;
        else if (_period == 180) _precent = 25000000000000000000;
